...
public class ConsultaDAO {
	...
	public Consulta pesquisarConsulta(int c)throws Exception{
		stmt = con.prepareStatement("SELECT * FROM T_SCP_CONSULTA INNER JOIN T_SCP_PSI_ONLINE "
				...
				"INNER JOIN T_SCP_USUARIO ON (T_SCP_USUARIO.CD_USUARIO= T_SCP_PACIENTE.CD_PACIENTE)" + 
				"INNER JOIN T_SCP_PSICOLOGO ON ( T_SCP_PSICOLOGO.CD_PSICOLOGO = T_SCP_PSI_ONLINE.CD_PSICOLOGO_ON) WHERE CD_CONSULTA = ?");
		stmt.setInt(1, c);
